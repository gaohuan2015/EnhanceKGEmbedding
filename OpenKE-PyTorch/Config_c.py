...
class Config(object):
    ...
    def link_prediction(self):
        ...
        for i in range(self.testTotal):
            ...
            print('[INFO] link_prediction: test_h.shape is:', self.test_h.shape)
            print('[INFO] link_prediction: content of res is:', res.shape)
...
# ============================================== 分析区域 =================================================
    ...
    def analysis_link_prediction(self):
        ...
        for i in range(self.testTotal):
            '''
            为什么一个测试样本要搞一个batch出来
            测试的batch里面包含了什么？
            对一个test ins, 一个batch里面包含了
            '''
            sys.stdout.write("%d\r" % (i))
            sys.stdout.flush()
            self.lib.getHeadBatch(self.test_h_addr, self.test_t_addr, self.test_r_addr)
